...
\begin{itemize}
    ...
    \item \textbf{Métricas de evaluación:} Accuracy, Precision, Recall, F1-score \parencite{powers2020evaluation} y ROC-AUC.
\end{itemize}
...
La regresión logística estima la probabilidad \parencite{hosmer2013applied} de que un desarrollador use IA dada una combinación lineal de variables predictoras:
...
\begin{itemize}
    \item $\sigma(z) = \frac{1}{1 + e^{-z}}$ es la función sigmoide \parencite{hosmer2013applied}.
...
Random Forest consiste en un conjunto de árboles de decisión entrenados sobre subconjuntos aleatorios de datos y variables para reducir la varianza del modelo \parencite{breiman2001random}
...
Sobre este pipeline se aplicó una búsqueda en malla (\texttt{GridSearchCV}) \parencite{bergstra2012random} utilizando
